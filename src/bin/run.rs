...
fn main() {
    ...
    let pt_mod = if polynomial_degree == 1024 { 15 } else { 16 };
    ...
    println!("db size is {}B", pt_mod * 2048 * 2048 * (1u64 << db_dim_1) * (1u64 << db_dim_2) / 8);
    println!("record size is {}B", pt_mod * (1 << mlwe_bit) / 8);
        
    run_module_pir_on_params(mlwe_bit, db_dim_1, db_dim_2, pt_mod as usize);
}
