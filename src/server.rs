...
////////////////////////////from here///////////////////////
	...
	let n = 1 << poly_len_log2;
    	let mut mlwe_vector = mlwe_to_mlwe(&a, n, n, n); // mlwedim, n, len

    	for i in 1..(poly_len_log2 - log_pt_byte) {
    	    mlwe_vector = mlwe_to_mlwe(&mlwe_vector, n >> i, n, n << i);
    	}
    	mlwe_vector
    }
    ...
    *const T: ToM512,
{
    ...
    pub fn perform_offline_precomputation_mlwe (
	&self, 
	mlwe_params: &'a Params,
	...
	t_exp: usize,
	db_cols: usize, 
	expansion_key_a: &[PolyMatrixNTT<'a>],
	...
	//client: &Client
    )->(Vec<(PolyMatrixNTT<'a>, Vec<PolyMatrixNTT<'a>>, Vec<Vec<usize>>)>, Vec<Vec<PolyMatrixNTT<'a>>>, Vec<PolyMatrixNTT<'a>>,  PolyMatrixNTT<'a>){
	...
	(packed_a_vec, decomp_a, hint_0s, double_query_a)
    }
	...
	expansion_table_neg: &[PolyMatrixNTT<'a>], 
	expansion_table_pos: &[PolyMatrixNTT<'a>], 
	decomp_a: &[Vec<PolyMatrixNTT<'a>>], 
	hint_0s: &[PolyMatrixNTT<'a>], 
	double_query_a: &PolyMatrixNTT<'a>, 
	packed_a_vec: &[(PolyMatrixNTT<'a>, Vec<PolyMatrixNTT<'a>>, Vec<Vec<usize>>)], 
	g_inv_b: &mut PolyMatrixRaw<'a>,
	g_inv_b_56: &mut PolyMatrixRaw<'a>, 
	double_query_b: &PolyMatrixNTT<'a>, 
	packed_query_col: &AlignedMemory64, 
	//client: &Client,
	
    ) ->(Vec<Vec<u8>>, Vec<u8>) {
	////////////////////////////////things to create before response////////////////////////////////

	let tmp_scale = 1<<(self.params.poly_len_log2 - mlwe_params.poly_len_log2 - 1);
	let rlwe_q_prime_1 = self.params.get_q_prime_1();
	let rlwe_q_prime_2 = self.params.get_q_prime_2();
...
///////////////////////////////response starts/////////////////////////////
	...
	gadget_invert_rdim(g_inv_b, &response_b_simple_rescaled, 1); // db * b1 ->gadget : g_invb
	...
	for num in 0..2{
	    ...
	    let mut tmp_rlwe_poly = PolyMatrixRaw::zero(&self.params, 1, 1);
...
///////////////////////////////////////////////////
	...
	for i in 0..2{
	    ...
	    mlwe_to_rlwe_a(&self.params, &mut input_poly_tmp, response_0_times_hint_double_vec[i].raw().as_slice().to_vec(), mlwe_params.poly_len_log2);
	    for z in 0..self.params.poly_len {
        	let val = barrett_reduction_u128(&self.params, input_poly_tmp.as_slice()[z] as u128 * tmp_scale as u128);
        	input_poly.data[z] += val;
        	if input_poly.data[z] >= self.params.modulus {
           	    input_poly.data[z] -= self.params.modulus;
        	}
    	    }
...
///////////////////////////////////////////////////////////////////
	...
	for num in 0..2{
	    ...
	    for j in 0..mlwe_params.poly_len{
		let val = barrett_reduction_u128(&self.params, last_mlwe_tmp_raw.as_slice()[j] as u128 * tmp_scale as u128);
		let idx = j*dimension + num;
		tmp_last_rlwe_poly.data[idx] += val;
		if tmp_last_rlwe_poly.data[idx] >= self.params.modulus {
		    tmp_last_rlwe_poly.data[idx] -= self.params.modulus;
		}
	    }
	}
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	let (packed_a_vec, decomp_a, hint_0s, double_query_a) = server.perform_offline_precomputation_mlwe (
	    &mlwe_params, 
	    &simple_params, 
	    dimension,
	    t_exp, 
...
////////////////////////////////online computation////////////////////////////////
    ...
    #[test]
    fn test_whole_no_fn(){ // start : row: 16384 / second : col:8192

	println!("creating server");

	////////first settings
	let mut params = params_for_scenario(1<<30, 1);
	params.pt_modulus = 1<<14;
	
	let rlwe_q_prime_1 = params.get_q_prime_1();
	let rlwe_q_prime_2 = params.get_q_prime_2();

	println!("{}, {}", rlwe_q_prime_1, rlwe_q_prime_2);

	//256MB: 3, 2 //1GB: 4, 3 //8GB: 5, 5
	params.db_dim_1 = 5;
	params.db_dim_2 = 5;

	let mut double_params = params.clone();
	double_params.pt_modulus = 1<<10;

	let mut mlwe_params = params.clone();
	mlwe_params.poly_len_log2 = 6;
	mlwe_params.poly_len = 1<<mlwe_params.poly_len_log2;

	let mut double_mlwe_params = mlwe_params.clone();
	double_mlwe_params.pt_modulus = 1<<14;
	
	let mut simple_params = mlwe_params.clone();
	simple_params.modulus = 1<<26;
	simple_params.modulus_log2 = 26;
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	for i in 0..db_rows{
	    for j in 0..db_cols{
		let value: u16 = rng.gen_range(129..=256); // 0 ~ 65535 중 임의의 값
        	matrix.push(value);
	    }
	}	
...
///////////////////////////////////////////////////////////////////
	...
	for ct in packed_b.iter() {
	    let res = ct.raw();
	    let res_switched = res.switch(params.modulus, params.modulus);//rlwe_q_prime_1, rlwe_q_prime_2);
	    packed_mod_switched_a.push(res_switched);
	}
	
	let res_b = last_packed.raw();
	let res_switched_b = res_b.switch(params.modulus, params.modulus);
	...
	for i in 0..mlwe_params.poly_len{
	    println!("{}, {}, {}", i, server.db()[(target_col*mlwe_params.poly_len+i)*db_rows + target_row] as u64, dec_res[0].data[i]);
	    assert_eq!(server.db()[(target_col*mlwe_params.poly_len+i)*db_rows + target_row] as u64, dec_res[0].data[i]);
	}
...
//	println!("total time: {:?}", end - start);
	...
	for i in 0..db_rows{
	    ...
	    for j in 0..db_cols{
		let value: u16 = j as u16;		
		
		//let value: u16 = rng.gen_range(65000..=65535); // 0 ~ 65535 중 임의의 값
...
///////////////////////////////////////////////////////////////////
    ...
    #[test]
    fn test_protocol_no_rescale(){ // start : row: 16384 / second : col:8192
	...
	let mut g_inv_a = PolyMatrixRaw::zero(&mlwe_params, dimension * 4, db_cols / mlwe_params.poly_len);
	let mut g_inv_b = PolyMatrixRaw::zero(&mlwe_params, 4, db_cols / mlwe_params.poly_len);
	...
	gadget_invert_rdim(&mut g_inv_a, &response_a_simple_raw_transposed, dimension);

	let g_inv_a_ntt = g_inv_a.ntt(); // hint 0
...
///////////////////////////////response starts/////////////////////////////
	...
	gadget_invert_rdim(&mut g_inv_b, &response_b_simple_transposed, 1); // db * b1 ->gadget : g_invb


	let g_inv_b_ntt = g_inv_b.ntt(); // response 0
...
///////////////////////////////////////////////////////////////////
	...
	let composed_a = &g_exp.ntt() * &decomposed_a.ntt(); // decrypted a

	for i in 0..params.poly_len{
	    //assert_eq!(response_a_simple_raw_transposed.submatrix(0, target_col, dimension, 1).data[i], composed_a.raw().data[i]);
	}
	...
	let composed_b = &g_exp.ntt() * &poly_temp_b.ntt(); //decrypted b

	for i in 0..mlwe_params.poly_len{
	    //assert_eq!(composed_b.raw().data[i], response_b_simple_transposed.submatrix(0, target_col, 1, 1).data[i]);
	}

	let decrypted_answer = decrypt_mlwe_batch(&params, &mlwe_params, dimension, &composed_a, &composed_b, &y_client.inner);
