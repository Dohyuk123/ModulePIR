...
use spiral_rs::{client::*, params::*, gadget::*};
...
use super::{client::*, lwe::LWEParams, measurement::*, params::*, server::*, packing::*};
...
pub fn test_module_pir_on_params_no_fn (mlwe_bit: u32) {
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	println!("rows: {} {}, cols: {} {}", params.db_dim_1, db_rows, params.db_dim_2, db_cols);
	...
	for i in 0..db_rows{
	    for j in 0..db_cols{
		let value: u16 = ( (i as u16) * (db_cols as u16) + j as u16 ) % 32768; // 0 ~ 65535 중 임의의 값
        	matrix.push(value);
		if ( j == 3 ) {
		    //println!("{}", value);
		}
...
////////////////////////////////client creates query////////////////////////////
	...
	let val_row: u32 = 10000;//rng.gen_range(0..(db_rows as u32));
	let val_col: u16 = 2;//rng.gen_range(0..((db_cols / mlwe_params.poly_len) as u16));
...
///////////////////////////////////////////////////////////////////
	...
	for i in 0..mlwe_params.poly_len{
	    ...
	    //println!("{}, {}", (target_col*mlwe_params.poly_len+i)*db_rows + target_row, dec_res[0].data[i]);
	}
...
pub fn run_module_pir_on_params_no_fn (mlwe_bit: u32) {
...
///////////////////////for keyswitching/////////////////////////
	...
	&mut ChaCha20Rng::from_entropy(), 
	&mut ChaCha20Rng::from_seed(pack_seed), 
	&mut client); // key switching keys for lwe to mlwe
...
////////////////////////////////client creates query////////////////////////////
	...
	// simple query
        let query_col = y_client.generate_query_mlwe( // query b -> b part
	    SEED_1, 
	    params.db_dim_1, 
	    mlwe_params.poly_len_log2, 
	    true, 
	...
	//double query
        let double_query_b = y_client.generate_query_double_mlwe( // col: 8192
	    &mlwe_params, 
	    SEED_0, 
	    dimension, 
	    params.db_dim_2,  
...
////////////////////////////////online computation////////////////////////////////
        ...
        let dec_res = y_client.decode_response_mlwe(
	    &mlwe_params, 
	    &packed_mod_switched_a, 
	    &res_switched_b, 
	    &g_exp_56_ntt, 
	    rlwe_q_prime_1, 
	    rlwe_q_prime_2
...
pub fn run_ypir_on_params<const K: usize>(
    ...
    println!("db row: {}, db rows padded : {}, db col: {},sqrt_n_bytes: {}, rlwe_q1: {}, rlwe_q2 : {}, lwe_q_primebits: {}", db_rows, db_rows_padded, db_cols, sqrt_n_bytes, rlwe_q_prime_1, rlwe_q_prime_2, lwe_q_prime_bits);
    println!("double_pir_mod: {}, simple_pir_mod: {}, params.modulus : {}", params.modulus_log2, (lwe_params.modulus as f64).log2().ceil(), params.modulus);
