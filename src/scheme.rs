...
pub fn run_new_pir_batched(
    ...
    let mlwe_params = params_for_scenario(num_items, item_size_bits);
    let measurement = match num_clients {
        1 => run_ypir_on_params::<1>(params, is_simplepir, trials),
        2 => run_ypir_on_params::<2>(params, is_simplepir, trials),
        3 => run_ypir_on_params::<3>(params, is_simplepir, trials),
...
pub fn run_new_pir_on_params<const K: usize>(
    params: Params,
    is_simplepir: bool,
    trials: usize,
    new_bits: usize,
    ...
    mlwe_params.poly_len_log2 = msg_bits;
    mlwe_params.poly_len = 1<<msg_bits;
    let db_rows = 1 << (params.db_dim_1 + params.poly_len_log2);
    let db_rows_padded = params.db_rows_padded();
    ...
    println!("db row: {}, db rows padded : {}, db col: {},sqrt_n_bytes: {}, rlwe_q1: {}, rlwe_q2 : {}, lwe_q_primebits: {}, double_pir_mod: {}, simple_pir_mod: {}", db_rows, db_rows_padded, db_cols, sqrt_n_bytes, rlwe_q_prime_1, rlwe_q_prime_2, lwe_q_prime_bits, params.modulus_log2, (lwe_params.modulus as f64).log2().ceil());
    println!("double_pir_mod: {}, simple_pir_mod: {}", params.modulus_log2, (lwe_params.modulus as f64).log2().ceil());
    ...
    // --
    println!("double_pir_mod: {}, simple_pir_mod: {}", params.modulus_log2, lwe_q_bits);
    let now = Instant::now();
    let pt_iter = std::iter::repeat_with(|| u8::sample());
    let y_server = NewServer::<u8>::new(&params, &mlwe_params, pt_iter, is_simplepir, false, true);
    debug!("Created server in {} us", now.elapsed().as_micros());
    debug!(
        "Database of {} bytes",
