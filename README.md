...
Options:
  -p, --polynomial-degree <POLYNOMIAL_DEGREE>
          ...
          Must be a power of 2, range: 8 to 1024
...
The database is configured as a matrix of size `(db_dim_1 * 2048) x (db_dim_2 * 2048)` with entries in modulus `2^15`.
...
```
db_size (bytes) = 15 * 2048 * 2048 * (2^db_dim_1) * (2^db_dim_2) / 8
```
...
The record size is determined by the polynomial degree:

```
record_size (bytes) = 15 * polynomial_degree / 8
```

For example, with the default `polynomial_degree = 128`, each record is `15 * 128 / 8 = 240 bytes`.
...
```bash
# Default (1 GB database, 128-byte records)
cargo run --release
